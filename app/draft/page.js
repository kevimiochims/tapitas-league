...
export default function DraftPage() {
  ...
  // Snake draft board: round par = ordem inversa
  const boardMatrix = useMemo(() => {
    return rounds.map(round => {
      const rowTeams = round % 2 === 0 ? [...teams].reverse() : [...teams]
      return rowTeams.map(team => {
        return seasonPicks.find(p => p.round === round && p.team === team) || null
      })
    })
                  ...
                  <table className="w-full border-separate border-spacing-1" style={{ minWidth: `${teams.length * 140}px` }}>
                    <thead>
                      <tr>
                        ...
                        {(rounds[0] % 2 === 0 ? [...teams].reverse() : [...teams]).map(team => (
                        ...
                        <tr key={rIdx}>
                          ...
                          {row.map((pick, cIdx) => (
                            <td key={cIdx} className="px-1 py-1">
                              {pick ? (
                                <div className="rounded-xl border border-white/5 bg-white/[0.03] p-2 hover:bg-white/[0.06] transition-all">
                                  <div className="mb-1 flex items-center justify-between gap-1">
                                    <span className="text-[9px] font-black text-slate-600">#{pick.pick}</span>
                                    <PosBadge pos={pick.position}/>
                                  </div>
                                  <div className="text-xs font-black text-white leading-tight">{pick.player}</div>
                                </div>
                              ) : (
