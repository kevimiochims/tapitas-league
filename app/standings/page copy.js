...
async function safeFetch(url) {
  try {
    ...
    console.log('JSON length:', Array.isArray(json) ? json.length : 'not array', url)
...
export default function StandingsPage() {
  console.log('StandingsPage montando...')
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      console.log('allTime length:', allTime.length)
        console.log('history length:', history.length)
        console.log('History primeira linha:', history[0])
      setAllTimeData(allTime)
      setHistoryData(history)
      console.log('History colunas:', Object.keys(history[0] || {}))
      console.log('History primeira linha:', history[0])
      if (allTime.length > 0) {
  ...
  const seasons = useMemo(() => {
    console.log('useMemo seasons rodando, historyData:', historyData.length)
    const s = new Set()
  ...
  const allTeams = useMemo(() => {
    console.log('useMemo allTeams rodando, allTimeData:', allTimeData.length)
    const t = new Set()
  ...
  const tableData = useMemo(() => {
    console.log('useMemo tableData rodando')
    let rows = []
  ...
  const chartData = useMemo(() => {
    console.log('useMemo chartData rodando')
  if (!chartTeam) return []
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/5 px-8 py-6">
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <div className="w-36">
                    <Select
                    value={chartScope}
                    onChange={(val) => setChartScope(val)}
