...
const ESPN_ABBR_MAP = { was: 'wsh' }
function getNFLTeamLogo(abbr) {
  if (!abbr || abbr === '' || abbr === '--') return null
  const raw = String(abbr).toLowerCase().trim()
  return `https://a.espncdn.com/i/teamlogos/nfl/500/${ESPN_ABBR_MAP[raw] || raw}.png`
}
                          ...
                          <div className="grid grid-cols-[1fr_60px_1fr] gap-1 md:gap-2 mb-2 items-center">
                            ...
                            <div className={`flex items-center justify-between rounded-2xl px-2 md:px-3 py-2 min-w-0 gap-2 ${home ? 'bg-white/[0.03] border border-white/5' : 'opacity-0'}`}>
                              ...
                              <div className="flex flex-col items-end min-w-0 flex-1">
                                <span className="text-[10px] font-semibold text-slate-400 truncate w-full text-right leading-tight">{home?.name ?? ''}</span>
                                <span className={`text-lg font-black leading-tight tabular-nums ${(home?.pts ?? 0) > 0 ? 'text-cyan-300' : 'text-slate-600'}`}>{home ? home.pts.toFixed(1) : '—'}</span>
                              </div>
                            </div>
                            ...
                            <div className={`flex items-center justify-between rounded-2xl px-2 md:px-3 py-2 min-w-0 gap-2 ${away ? 'bg-white/[0.03] border border-white/5' : 'opacity-0'}`}>
                              <div className="flex flex-col items-start min-w-0 flex-1">
                                <span className="text-[10px] font-semibold text-slate-400 truncate w-full leading-tight">{away?.name ?? ''}</span>
                                <span className={`text-lg font-black leading-tight tabular-nums ${(away?.pts ?? 0) > 0 ? 'text-cyan-300' : 'text-slate-600'}`}>{away ? away.pts.toFixed(1) : '—'}</span>
                              </div>
                        ...
                        <React.Fragment key={i}>
                          <div className="grid grid-cols-[1fr_60px_1fr] gap-1 md:gap-2 mb-2 items-center">
                            ...
                            <div className={`flex items-center justify-between rounded-2xl px-2 md:px-3 py-2 min-w-0 gap-2 ${home ? 'bg-white/[0.02] border border-white/[0.03]' : 'opacity-0'}`}>
                              ...
                              <div className="flex flex-col items-end min-w-0 flex-1">
                                <span className="text-[10px] font-semibold text-slate-500 truncate w-full text-right leading-tight">{home?.name ?? ''}</span>
                                <span className={`text-sm font-black leading-tight tabular-nums ${(home?.pts ?? 0) > 0 ? 'text-slate-300' : 'text-slate-600'}`}>{home ? home.pts.toFixed(1) : '—'}</span>
                              </div>
                            </div>
                            ...
                            <div className={`flex items-center justify-between rounded-2xl px-2 md:px-3 py-2 min-w-0 gap-2 ${away ? 'bg-white/[0.02] border border-white/[0.03]' : 'opacity-0'}`}>
                              <div className="flex flex-col items-start min-w-0 flex-1">
                                <span className="text-[10px] font-semibold text-slate-500 truncate w-full leading-tight">{away?.name ?? ''}</span>
                                <span className={`text-sm font-black leading-tight tabular-nums ${(away?.pts ?? 0) > 0 ? 'text-slate-300' : 'text-slate-600'}`}>{away ? away.pts.toFixed(1) : '—'}</span>
                              </div>
